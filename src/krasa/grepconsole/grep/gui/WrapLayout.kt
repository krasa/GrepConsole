...
open class WrapLayout : FlowLayout {
    ...
    override fun layoutContainer(target: Container) {
        synchronized(target.treeLock) {
            ...
            moveComponents(
                target, insets.left + hgap, y, maxwidth - x, rowh,
    ...
    private fun moveComponents(
        ...
        for (i in rowStart until rowEnd) {
            ...
            if (m.isVisible) {
                ...
                val w =
                    if (fillWidth) floor(max(m.preferredSize.width, m.minimumSize.width) * expand).toInt() else m.width
                if (ltr) {
